...
18. When "travel_resolution" is present, narrate exactly that route and outcome. For "arrived", use narration_mode and elapsed_minutes as a transition/timeskip; do not invent a different destination or duration. For "interrupted", stop at stopped_at and open a scene from interrupted_leg, its danger and tags. For "blocked", keep the character at the origin and explain the obstacle in-story. The engine owns location and clock changes.
